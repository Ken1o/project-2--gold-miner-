# making more gemstone for variety
import os
import math
import pygame
...
def display_time(time):
    txt_timer = game_font.render(f"Remaining Time: {time}", True, BLACK)
    screen.blit(txt_timer, (950, 50))

def display_game_over():
    game_font = pygame.font.SysFont("arialrounded", 60)
    txt_game_over = game_font.render(game_result, True, BLACK)
    rect_game_over = txt_game_over.get_rect(center=(int(screen_width / 2), int(screen_height / 2))) # setting to the middle of the screen
...
# game over related variable
game_result = None # game result
total_time = 60 # total time of game

start_ticks = pygame.time.get_ticks() # getting current tick
...
while running:
    ...
    display_time(total_time - int(elapsed_time)) # display time

    # if time is below 0 game over
